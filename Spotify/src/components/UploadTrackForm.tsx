...
import { uploadTrackSchema } from '@/utils/validation'
import { useUploadTrackMutation } from '@/redux/services/uploadApi'
import { useGetArtistsQuery } from '@/redux/services/artistApi'
import { useGetAlbumsByArtistQuery } from '@/redux/services/albumApi'
...
export default function UploadTrackForm() {
    ...
    const { data: albums } = useGetAlbumsByArtistQuery(artistId, {
        skip: !artistId,
    })
    ...
    const onSubmit = async (data: UploadTrackFormData) => {
        try {
            ...
            const file = data.file?.[0] // 🛠️ Lấy file từ FileList
            if (!file) {
